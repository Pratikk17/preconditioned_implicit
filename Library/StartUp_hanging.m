function [M,M_H,M_E,A_H, A_E,invM_H, invM_E, A_Hi, A_He, A_Ei, A_Ee]...
    = StartUp_hanging(mesh_parameters,options,triangluar_parameters);
% % Calculates the mass matrix, stiffness matrix for the Maxwell's equation
% on the considered mesh with hanging nodes 
...
path_upwind = 'matrices/rectangular_strip_meshes/upwind';

Globals2D;

%%====== create a non-conforming interface by refinement.. 
% we first refine the whole mesh into min(refine_coarse,refine_fine) times
% and then refine only coarse or fine mesh depending upon which is larger

%=== create mesh parameters for the mesh to start with.
[mesh_parameters_start,refine_times]=start_mesh(mesh_parameters);

mesh_name = strcat('Nc_', num2str(mesh_parameters.Nc), '_c_', num2str(mesh_parameters_start.Nrefine_coarse), '_f_', num2str(mesh_parameters_start.Nrefine_fine),'_r_',...
        num2str(mesh_parameters.ratio_x), '.neu');
mesh_name=generate_mesh_script(mesh_parameters_start);
[Nv, VX, VY, K, EToV, BCType] = MeshReaderGambitBC2D(mesh_name);
ini_elem=K;
StartUp2D;    % construct grid and metric
BCType = Wall*(EToE==((1:K)'*ones(1,Nfaces))); % make boundary conditions all Wall type

for i=1:refine_times
    [refine_array]=refine_parameters(mesh_parameters,mesh_parameters_start,ini_elem,K);
    refineflag = zeros(K,1);
    refineflag(refine_array) = 1;
...
if exist(mat_file_stiff, 'file') == 2                                      % File exists
    ...
    [Ss, Sf] = Maxwell_Stif_Matrix2D_hanging();
    A = Ss + Sf;
...
A = A([indHx0 indHy0 indEz0], [indHx0 indHy0 indEz0]);
M = M([indHx0 indHy0 indEz0], [indHx0 indHy0 indEz0]);
invM = invM([indHx0 indHy0 indEz0], [indHx0 indHy0 indEz0]);
