% Files to be provided to Nick

% These script creates and stores matrices A, At, M, sqrtM, which are required for
% calculation of eigenvalues and field of values. It then
% plots the eigenvalues, region bounding FOV and a
...
for p=1:length(dT)
    ...
    if options.plot_eig==1
        filename_At_exist = sprintf('matrices/gmsh_square_meshes/Nick/At_polydeg_%d_dt_%s.mat',...
                         N,num2str(dt));
        filename_At = sprintf('matrices/gmsh_square_meshes/Nick/At_polydeg_%d_dt_%s.mat',...
                   N,num2str(dt));
        if exist(filename_At_exist,'file')==2
            load([filename_At])
            disp('Matrices At, At_M, sqrt_ME and sqrt_ME_inv loaded');
        else
            ...
            save([filename_At_exist],'At','At_M','sqrt_ME','sqrt_ME_inv')
            disp('Matrices At, At_M, sqrt_ME and sqrt_ME_inv Saved');
            % Note that eigen values of At and At_M=sqrt_ME*At*sqrt_ME_inv are same. Hence we consider only eigenvalues of At.
        end
        %
        filename_eig_exist = sprintf('matrices/gmsh_square_meshes/Nick/eig_At_polydeg_%d_dt_%s.mat',...
                         N,num2str(dt));
        filename_eig = sprintf('matrices/gmsh_square_meshes/Nick/eig_At_polydeg_%d_dt_%s.mat',...
                   N,num2str(dt));
        if exist(filename_eig_exist,'file')==2
            load([filename_eig])
            disp('Eigenvalues loaded');
        else
            At_eigen=eig(At);
            % Note that eigen values of At and At_M=sqrt_ME*At*sqrt_ME_inv are same. Hence we consider only eigenvalues of At.
            save([filename_eig],'At_eigen')
...
%
        filename_fov_exist = sprintf('matrices/gmsh_square_meshes/Nick/FOV_At_polydeg_%d_dt_%s.mat',...
            N,num2str(dt));
        filename_fov = sprintf('matrices/gmsh_square_meshes/Nick/FOV_At_polydeg_%d_dt_%s.mat',...
            N,num2str(dt));
        if exist(filename_fov_exist,'file')==2
            load([filename_fov])
            disp('FOV loaded');
        else
            At_FOV = wber3(At_M,10,'--b');                                % calculating FOV
            save([filename_fov],'At_FOV')
            disp('FOV Saved');
        end
